...
#              at the wx.App level. This is useful for resolving cases where multiple views
#              may want to respond to an event (say, wx.ID_COPY) and where you also want a
#              "default" handler for the event (and UI update status) when there is no active
...
class AppEventHandlerMixin(object):
    ...
    To work with this system, views must call Add(UI)HandlerForID(ID, handlerFunc)
