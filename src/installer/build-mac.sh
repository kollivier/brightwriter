...
if [ "$skipmac" != "yes" ]; then
     ...
     dir=$MAC_BUILD/trunk/brightwriter
     ...
     mkdir -p $DMG_DIR
     scp -r "$MAC_HOST:$dir/installer/dist/BrightWriter.app "  $DMG_DIR
     #cd $OLDDIR
     ...
     hdiutil create -srcfolder $DMG_DIR -volname "BrightWriter" -imagekey zlib-level=9 $DMG_NAME
     ...
     #cp -r deliver/BrightWriter.app /Volumes/BrightWriter
     
     #hdiutil detach /Volumes/BrightWriter
