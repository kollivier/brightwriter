...
if form.has_key("term"):
    ...
    if 1:
        ...
        query = PyLucene.QueryParser.parse(form["term"], field, analyzer)
        ...
        for num in range(startitem-1, startitem-1 + numresults_perpage):
            hit = None
            if num < hits.length():
                try:
                    hit = hits.doc(num)
        ...
        if hits.length() > numresults_perpage:
            result = divmod(hits.length(), numresults_perpage)
            pages = result[0]
            if not result[1] == 0:
                pages = pages + 1
