...
plugin_info = {"Name":"quiz", "FullName":"Quiz", "Directory":"EClass", "Extension":["quiz"], "Mime Type": "","Requires":""}
...
class HTMLPublisher(plugins.BaseHTMLPublisher):
	...
	def GetFilename(self, filename):
		...
		filename = os.path.splitext(filename)[0] #string.replace(filename, ".quiz", "")
...
class EditorDialog(wxDialog):
	def __init__(self, parent, item):
		...
		if len(self.item.content.filename) > 0:	
			if not os.path.exists(os.path.join(self.parent.CurrentDir, "EClass", self.item.content.filename)):
				self.quiz.SaveAsXML(os.path.join(self.parent.CurrentDir, "EClass", self.item.content.filename))

			self.quiz.LoadPage(os.path.join(self.parent.CurrentDir, "EClass", self.item.content.filename))
			for item in self.quiz.items:
			...
			while os.path.exists(os.path.join(self.parent.pub.directory, "EClass", filename)):
				filename = self.item.content.name[:27] + " " + `counter` + ".quiz"
				counter = counter + 1
