...
plugin_info = {"Name":"eclass", "FullName":"EClass Page", "Directory":"EClass", "Extension":["ecp"], "Mime Type": "","Requires":""}
...
class HTMLPublisher(plugins.BaseHTMLPublisher):
	...
	def GetFilename(self, filename):
		...
		filename = os.path.splitext(filename)[0] #filename = string.replace(filename, ".ecp", "")
	...
	def __init__(self, parent, item):
		...
		if isinstance(item, conman.conman.ConNode):
			...
			if len(self.filename) > 0:
				if not os.path.exists(os.path.join(self.CurrentDir, "EClass", self.filename)):
					self.page.SaveAsXML(os.path.join(self.CurrentDir, "EClass", self.filename))

				try:
					self.page.LoadPage(os.path.join(self.CurrentDir, "EClass", self.filename))
			...
			self.page.name = item.content.name
		else:
...
<BODY>
	...
	def btnOKClicked(self,event):
		...
		if isinstance(self.item, conman.conman.ConNode):
			if len(self.filename) > 0:
				try:
					self.page.SaveAsXML(os.path.join(self.parent.CurrentDir, "EClass", self.filename),self.mainform.encoding)
				except IOError, e:
